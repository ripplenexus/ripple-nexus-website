...
import { submitForm } from '../../src/services/api';  // API service
import { useNavigate } from 'react-router-dom';

const Contacts: React.FC = () => {
  const navigate = useNavigate();  // Use this to redirect to the confirmation page
  const [formData, setFormData] = useState({
    name: '',
    surname: '',
    email: '',
    message: '',
    service: ['Web Development']  // Default selection
  });
  ...
  return (
    <div>
      ...
      <div className="relative top-[-150px] flex flex-col lg:flex-row justify-center lg:justify-between items-start w-full max-w-[1200px] mx-auto">
        <form onSubmit={handleSubmit} className="flex flex-col w-full lg:w-3/5 bg-white shadow-md p-8 rounded-md mb-8 lg:mb-0">
          ...
          <input
            ...
            className="mb-3 px-2 py-2 border border-[#D9D9D9] rounded-md"
          />
          ...
          <button className="w-[6.5em] h-[3.5em] mt-2 bg-[#457FD7] text-white rounded-md">Submit</button>
        </form>
        ...
        <div className="flex flex-col w-full lg:w-2/5 p-[46px] lg:p-[52px] bg-[#103F6D] shadow-lg border-[1.3px] border-[#00000029] rounded-md h-[640px]">
          <h2 className="text-center text-white font-semibold text-[21.1px] mb-[17.3px]">Contact Information</h2>
